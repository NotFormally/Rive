    ...
    <>
      <header className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10">
        ...
        <div className="px-8 flex items-center gap-6 text-sm font-medium border-t border-zinc-100 dark:border-zinc-800">
          <a href="/dashboard/food-cost" className="py-3 border-b-2 border-transparent text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">{tr("tab_overview")}</a>
          <a href="/dashboard/food-cost/ingredients" className="py-3 border-b-2 border-transparent text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">{tr("tab_ingredients")}</a>
          <a href="/dashboard/food-cost/recipes" className="py-3 border-b-2 border-transparent text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors">{tr("tab_recipes")}</a>
          <a href="/dashboard/food-cost/invoices" className="py-3 border-b-2 border-indigo-600 text-indigo-600 dark:text-indigo-400">{tr("tab_invoices")}</a>
        </div>
      </header>
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
            <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">{t("history_title")}</h2>
            <div className="text-sm text-zinc-500 flex items-center gap-1">
              {t("invoices_count", { count: invoices.length })}
            </div>
          </div>
          
          <div className="p-0">
             {loading ? (
                <div className="p-10 text-center text-zinc-500">{t("loading_history")}</div>
             ) : invoices.length === 0 ? (
                      ...
                      <div key={invoice.id} className="p-4 sm:p-6 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                         <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-3 text-sm text-zinc-500">
                               ...
                               <span className="text-zinc-600 dark:text-zinc-400">{t("items_detected", { count: invoice.invoice_items?.[0]?.count || 0 })}</span>
                            </div>
                         </div>
                         ...
                         <div className="flex flex-col sm:items-end gap-1 w-full sm:w-auto">
                            ...
                            {invoice.top_items && invoice.top_items.length > 0 && (
                               <div className="text-xs text-zinc-400 truncate max-w-[200px] sm:max-w-xs">
                                  {t("includes", { items: invoice.top_items.slice(0, 2).join(', ') })}
                               </div>
                            )}
                         </div>
                      </div>
