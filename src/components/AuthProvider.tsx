...
type AuthContextType = {
  ...
  loading: boolean;
  restaurants: RestaurantMembership[]; // i18n-ignore
  switchRestaurant: (restaurantId: string) => Promise<void>; // i18n-ignore
...
const ACTIVE_RESTAURANT_KEY = 'rive_active_restaurant_id';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Load a specific restaurant's profile, settings, scores
  const loadRestaurantData = async (restaurantId: string, memberRole: MemberRole) => {
    setRole(memberRole);
    ...
    if (profileData) {
      ...
      const [settingsResult, scoreResult, healthResult] = await Promise.all([
        supabase.from("restaurant_settings").select("*").eq("restaurant_id", profileData.id).single(),
        supabase.from("restaurant_intelligence_score").select("score, level").eq("restaurant_id", profileData.id).single(),
        supabase.from("restaurant_health_scores").select("total_score, grade").eq("restaurant_id", profileData.id).maybeSingle(),
      ]);
  ...
  const loadProfile = async (userId: string) => {
    try {
      ...
      const { data: memberships, error: memberError } = await supabase
        ...
        .select("restaurant_id, role, restaurant_profiles!inner(restaurant_name, logo_url, address)")
        .eq("user_id", userId)
        .not("accepted_at", "is", null);

      if (memberError) {
        console.error("[AuthProvider] DB Error loading memberships:", memberError);
      }
      ...
      // Build restaurants list for the switcher
      const restaurantList: RestaurantMembership[] = memberships.map((m: any) => ({
        restaurant_id: m.restaurant_id,
        restaurant_name: m.restaurant_profiles?.restaurant_name || 'Unknown',
        logo_url: m.restaurant_profiles?.logo_url || null,
        address: m.restaurant_profiles?.address || '',
        role: m.role as MemberRole,
      }));
      setRestaurants(restaurantList);

      // Determine which restaurant to load: localStorage > first membership
      let savedId: string | null = null;
      try { savedId = localStorage.getItem(ACTIVE_RESTAURANT_KEY); } catch {}
      const target = restaurantList.find(r => r.restaurant_id === savedId) || restaurantList[0];

      // 2. Load the active restaurant's data
      await loadRestaurantData(target.restaurant_id, target.role);
    } catch (err) {
  ...
  const switchRestaurant = async (restaurantId: string) => {
    const target = restaurants.find(r => r.restaurant_id === restaurantId);
    if (!target) return;
    try { localStorage.setItem(ACTIVE_RESTAURANT_KEY, restaurantId); } catch {}
    await loadRestaurantData(target.restaurant_id, target.role);
  };

  const signOut = async () => {
    await supabase.auth.signOut({ scope: 'local' });
    try { localStorage.removeItem(ACTIVE_RESTAURANT_KEY); } catch {}
    setUser(null);
    setProfile(null);
    setRole(null);
    setSettings(null);
  ...
  return (
    <AuthContext.Provider value={{ user, profile, role, settings, usage, subscription, intelligenceScore, intelligenceLevel, healthScore, healthGrade, loading, restaurants, switchRestaurant, signOut, refreshSettings, refreshProfile }}>
      {children}
