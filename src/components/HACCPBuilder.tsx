...
export function HACCPBuilder() {
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      alert("Modèle HACCP sauvegardé avec succès !");
    }, 1000);
  };
      ...
      <div className="flex items-start gap-4 justify-between flex-col md:flex-row">
        <div>
          <h1 className="text-3xl font-outfit font-bold text-white mb-2 tracking-tight">
            Constructeur de Formulaires HACCP
          </h1>
          <p className="text-white/60 text-sm max-w-xl leading-relaxed">
            Créez des audits sur mesure pour votre établissement (Réception, Refroidissement, Nettoyage). 
            Ces modèles généreront automatiquement des formulaires conformes pour vos équipes.
          </p>
        </div>
        <button
          onClick={handleSave}
        ...
        >
          ...
          {isSaving ? "Sauvegarde..." : "Enregistrer le modèle"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-[#1A1A1A] border border-white/10 rounded-2xl p-6 space-y-4">
            <div>
              <label className="block text-sm font-medium text-white/80 mb-1.5">Nom du modèle (ex: Réception Poisson)</label>
              <input 
                type="text" 
                value={templateName}
                onChange={(e) => setTemplateName(e.target.value)}
                placeholder="Exénommez ce modèle de contrôle..."
            ...
            <div>
              <label className="block text-sm font-medium text-white/80 mb-1.5">Description (Optionnel)</label>
              <textarea 
                value={description}
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-outfit font-medium text-white/90">Champs du formulaire ({fields.length})</h3>
            ...
            {fields.length === 0 ? (
              <div className="border-2 border-dashed border-white/10 rounded-2xl p-12 text-center flex flex-col items-center justify-center bg-[#1A1A1A]/50">
                ...
                <p className="text-white/60 font-medium mb-1">Aucun champ ajouté</p>
                <p className="text-white/40 text-sm">Utilisez le panneau de droite pour ajouter des contrôles de température, de texte ou des photos.</p>
              </div>
            ) : (
        ...
        <div className="lg:col-span-1">
          <div className="bg-[#1A1A1A] border border-white/10 rounded-2xl p-5 md:sticky md:top-6">
            <h3 className="text-sm font-outfit font-semibold text-white/90 mb-4 uppercase tracking-wider">Ajouter un champ</h3>
            <div className="space-y-2 grid grid-cols-2 lg:grid-cols-1 gap-2">
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-white/90">Nombre</div>
                  <div className="text-xs text-white/50 hidden md:block">Température, quantité...</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-white/90">Oui / Non</div>
                  <div className="text-xs text-white/50 hidden md:block">Nettoyé, Conforme...</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-white/90">Texte court</div>
                  <div className="text-xs text-white/50 hidden md:block">Observations, lot...</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-white/90">Preuve Photo</div>
                  <div className="text-xs text-white/50 hidden md:block">Photo d'étiquette...</div>
                </div>
              </button>
            </div>
          </div>
