        ...
        <div className="px-8 md:px-12 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {TRIAL_FEATURES.map((feature) => (
              <div
                key={feature.label}
                className="flex items-start gap-3 bg-white rounded-xl p-3 border border-slate-100 hover:-translate-y-0.5 transition-transform duration-300"
              >
                <div className="bg-[#2E4036]/10 p-1.5 rounded-lg shrink-0 mt-0.5">
                  <feature.icon className="w-4 h-4 text-[#2E4036]" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-jakarta font-bold text-sm text-[#1A1A1A] leading-tight mb-0.5 break-words">{feature.label}</p>
                  <p className="font-outfit text-[11px] text-slate-400 leading-tight">{feature.desc}</p>
                </div>
              </div>
            ))}
          </div>
